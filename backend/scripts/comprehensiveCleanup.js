...
async function comprehensiveCleanup() {
  try {
    ...
    // Get admin user
    const admin = await User.findOne({ email: 'admin@example.com' }).select('_id name');
    if (!admin) {
    ...
    if (beforeCounts.products + beforeCounts.customers + beforeCounts.sales === 0) {
    ...
    if (afterCounts.products + afterCounts.customers + afterCounts.sales === 0) {
