...
module.exports = override(
  ...
  // Force set the public URL and API URL for production builds
  (config) => {
    // Set the correct public URL for production builds
    if (process.env.NODE_ENV === "production") {
      config.output.publicPath =
        "https://businesscrmfrontend-kouc2wfhi-simons-projects-94c78eac.vercel.app/";
    }
    ...
    if (postcss) {
      ...
      if (process.env.NODE_ENV === "production") {
        ...
        if (Array.isArray(postcss.options.postcssOptions.plugins)) {
          postcss.options.postcssOptions.plugins.push(
